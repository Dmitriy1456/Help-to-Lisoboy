...
class WebCrawler:
    ...
    async def fetch(self, session, url):
        # Имитируем поведение человека: задержка перед кликом
        await asyncio.sleep(random.uniform(1.0, 3.0))

        # Заголовки, которые максимально похожи на реальный браузер
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8',
            'Accept-Language': 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7',
            'Referer': 'https://www.google.com/',  # Важно: якобы мы пришли из поиска
            'DNT': '1',
        }
    ...
    async def run(self):
        ...
        # Используем одну сессию на все запросы (TCP-reuse)
        async with aiohttp.ClientSession() as session:
            # Работаем в ОДИН поток (concurrency=1), чтобы не злить систему защиты
...
if __name__ == "__main__":
    # ПОПРОБУЙТЕ СНАЧАЛА ЭТОТ URL. Если он сработает — ваш код верный, а дело в защите Википедии.
    TEST_URL = "https://docs.python.org/3/tutorial/introduction.html"

    # Если все же нужна Википедия, попробуйте этот (мобильная версия + другая статья):
    # TEST_URL = "https://en.m.wikipedia.org/wiki/Python_(programming_language)"
